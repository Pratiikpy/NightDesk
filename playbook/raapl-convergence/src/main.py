"""Entry point for the rAAPL Off-Hours Convergence Playbook (backtest path).

backtest_support: full → the platform injects runtime.evaluation_mode="historical"
on /api/v1/playbook/run. We fetch rAAPL spot bars, replay the deterministic
convergence strategy, and emit the graded metrics.
"""
import math
...
def run() -> None:
    ...
    symbols = cfg.get("trading_symbols") or ["RAAPLUSDT"]
    symbol = symbols[0]

    bars = data.crypto.spot.kline(symbol=symbol, interval="1h", limit=1000)
