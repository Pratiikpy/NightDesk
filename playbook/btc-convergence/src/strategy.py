"""Deterministic mean-reversion (convergence) strategy.

Long-only: opens when price has stretched far below its recent path (a temporary
dislocation), exits when price reverts back toward the path.
"""
...
class ConvergenceStrategy(Strategy):
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        if self._position == "NONE":
            if z <= -float(self.cfg.entry_z):  # stretched below path → expect reversion up
                self._submit(instrument.id, OrderSide.BUY, qty)
                self._position = "LONG"
        elif self._position == "LONG":
            if z >= -float(self.cfg.exit_z):  # reverted toward path → take it
