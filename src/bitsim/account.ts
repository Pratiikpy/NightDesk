// Virtual account: cash, spot units, signed perp positions, realized/unrealized PnL,
// fees, funding. Cash-settled accounting (PnL realized into cash on close).
...
// Verified live (verification-log V5): spot 0.1% taker, perp 0.06% taker.
...
export class Account {
  ...
  applyPerpFill(symbol: string, side: Side, qty: number, price: number, leverage = 1): number {
    ...
    if (pos.qty === 0 || sameDir) {
      ...
      // reducing or closing (possibly flipping)
  ...
  /** Apply one funding settlement. Long pays when rate>0 (cash out), short receives. */
