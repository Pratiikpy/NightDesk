...
export async function exportFirewallUniverse(): Promise<void> {
  ...
  // Emit a plain TS data module (not JSON) so the serverless function imports it like any other module —
  // no JSON-import assertion, no runtime fs/path. The bundler inlines it.
  const file = join(OUT, "firewall-universe.ts");
  const content =
    `// AUTO-GENERATED by \`npm run firewall:export\` — do not edit by hand.\n` +
    `export const snapshotAt = ${JSON.stringify(new Date(snap.ts).toISOString())};\n` +
    `export const universe = ${JSON.stringify(universe, null, 2)};\n`;
