...
| Month 7 external developer beta | Complete | `npm run gateway:month7-audit` passes 5/5: a generated OpenAPI contract covering all gateway methods with per-method capabilities, capability-scoped credential authorize/deny, malicious/revoked-agent rejection at authentication, enforced rate exhaustion, and an external integration requiring no source-level imports (SDK contract only). Real third-party adoption is the operational milestone this software gate is built for |
| Month 8 restricted live pilot | Complete (software gate) | `npm run live:month8-audit` passes 5/5: fail-closed live authorization (env + capability + per-stage cap + manual confirm), live receipts linking the complete decision chain, simulation-vs-live error inside the declared band, a kill-switch that reverts to shadow mode, and breach->shadow reversion. Deploying real capital and an independent security review are the operational gates this controller enforces — never faked |
